...
public class Activitythree extends Activity {
    ...
    final ArrayList<String> ing_list = new ArrayList<String>(); // 진행중인 리스트
    final ArrayList<Integer> match_index = new ArrayList<Integer>(); // 각 항목의 모든 리스트 에서의 인덱스
    int ing_index = 0; // ing_list의 인덱스
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        connectThread th = new connectThread();     //스레드 실행
        try {
            th.start();
        }
        catch(Exception e)
    ...
    class connectThread extends Thread{
        public void run(){
            ...
            try {
                ...
                sock.connect(new InetSocketAddress("192.168.123.131", port), 5000);     //본인 IP입력
                if(!sock.isConnected()){
                }
                else{
                    try{
                        read(sock);
                        String text = send(sock, "2");
                        if(text.equals("2")){
                            ...
                            if(num!=0)
                            {
                                for(int i=0;i<num;i++)
                                {
                                    ...
                                    final int sect_index = Integer.parseInt(get_index);	//투표인덱스
                                        runOnUiThread(new Runnable() {      //스레드에서 UI 갱신해주는 method = runOnUiThread
                                            @Override
                                            public void run() {
                                                ing_list.add(message);      //ing_list 에 일단 받아온 투표 제목들을 넣어줌
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    for(int i = 0; ; i++){
                        ...
                        output_list[i] = ing_list.get(i);           //input_list에 넣어둔 투표리스트들을 output_list로 옮기기
                        output_index[i] = match_index.get(i);          //output_index에 투표리스트 값이 들어가는건가??!!
                    ...
                    final int finalIng_index = ing_index;
                        ...
                        public void onClick(View v){
                            ...
                            for(int i = 0; i < 10; i++){
                                ...
                                if(start_index[0] + i + 1 <= finalIng_index){
                                    output_list[i] = ing_list.get(start_index[0] + i);
